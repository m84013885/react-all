...
const webpackConfig = {
  ...
  entry: {},
  output: {
    path: path.resolve(__dirname, '../../statics/activityWeb/activityThanksgiving/assets'),
  ...
  module: {
    ...
    },
    {
      ...
      use: [{ loader: `url-loader?limit=1&name=[name].[ext]&outputPath=/images&publicPath=${publicPath}/images` }],
      include: [path.resolve(__dirname, 'app')],
      exclude: [nodeModuleDir]
    }
    ]
  ...
  plugins: [
    ...
    new AssetsRelacePlugin([
      path.resolve(__dirname, '../../../remix-server/templates/activity/activity18Double11.html'),
      path.resolve(__dirname, '../../../remix-server/templates/activity/activity18Double11Enter.html')
    ])
  ],
  mode: 'production'
}
routers.map((item, index) => {
  ...
  // const routerScript = [path.resolve(__dirname, `app/router/${template}/index.js`), path.resolve(__dirname, `app/utils/console.js`)]
